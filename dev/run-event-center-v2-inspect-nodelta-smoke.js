...
const INSPECT_OPTION_ID = 'inspect';
  ...
  return {
    ...
    eventV2: {
      ...
      openEvents: [{
        ...
        category: 'care',
        severity: 'warning',
        source: 'event-center-v2-inspect-nodelta-smoke',
...
function runSmoke() {
  const state = createState('evt_v2_inspect_nodelta_001');
  const beforeStatus = cloneJson(state.status);
  const beforeV1HistoryLength = state.events.history.length;

  const resolved = Bridge.resolveEventCenterV2PilotEvent(state, INSPECT_OPTION_ID, {
    now: state.simulation.nowMs + 1000
  });

  assert.strictEqual(resolved.ok, true, 'inspect should resolve through V2');
  assert.strictEqual(state.eventV2.openEvents.length, 0, 'openEvents should be empty after inspect resolve');
  assert.strictEqual(state.eventV2.history.length, 1, 'inspect should create one history entry');
  ...
  assert.strictEqual(historyEntry.selectedOption, INSPECT_OPTION_ID, 'selected option should be inspect');
  ...
  assert.strictEqual(historyEntry.appliedDelta.applied, false, 'inspect must not apply real status delta');
  assert.strictEqual(historyEntry.appliedDelta.reason, 'diagnostic_only', 'inspect should document diagnostic_only reason');
  assert.deepStrictEqual(historyEntry.appliedDelta.deltas, [], 'inspect appliedDelta.deltas should be empty');

  assert.deepStrictEqual(state.status, beforeStatus, 'inspect should not mutate status values');
  ...
  assert.strictEqual(resolved.applyDeltaStoredAsPilotPreview, true, 'result should mark inspect as noDelta preview');
  ...
  assert.strictEqual(reloadState.eventV2.history[0].selectedOption, INSPECT_OPTION_ID, 'reload clone should keep inspect history');
  assert.deepStrictEqual(reloadState.status, beforeStatus, 'reload clone should keep unchanged status');

  const duplicate = Bridge.resolveEventCenterV2PilotEvent(reloadState, INSPECT_OPTION_ID, {
    now: state.simulation.nowMs + 2000
  });
  assert.strictEqual(duplicate.ok, false, 'duplicate inspect resolve should be blocked after open event removal');
  assert.strictEqual(reloadState.eventV2.history.length, 1, 'duplicate inspect resolve must not duplicate history');

  const summary = {
    ok: true,
    mode: 'event_center_v2_inspect_nodelta_smoke',
    eventId: PILOT_EVENT_ID,
    selectedOption: INSPECT_OPTION_ID,
    openEventsAfterResolve: state.eventV2.openEvents.length,
    historyCountAfterResolve: state.eventV2.history.length,
    inspectNoDeltaReason: historyEntry.appliedDelta.reason,
