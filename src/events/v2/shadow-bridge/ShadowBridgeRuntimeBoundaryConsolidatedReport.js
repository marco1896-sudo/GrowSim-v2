...
(function initShadowBridgeRuntimeBoundaryConsolidatedReport(globalScope) {
  const Formatter = (globalScope && globalScope.ShadowBridgeRuntimeBoundaryReportFormatter) ||
    (typeof require !== 'undefined' ? require('./ShadowBridgeRuntimeBoundaryReportFormatter.js') : null);
  ...
  function normalizeDiagnosticsReport(checkResult) {
    const parsed = checkResult && checkResult.parsed ? checkResult.parsed : {};
    return {
      ok: Boolean(checkResult && checkResult.ok && parsed.ok === true && parsed.status === 'pass'),
  ...
  function normalizeShadowOnlyRuntimeBoundaryHarness(checkResult) {
    const parsed = checkResult && checkResult.parsed ? checkResult.parsed : {};
    return {
      ok: Boolean(checkResult && checkResult.ok && parsed.ok === true && parsed.status === 'pass'),
      labels: parsed.labels || {}
  ...
  function normalizeCombinedReport(checkResult) {
    const parsed = checkResult && checkResult.parsed ? checkResult.parsed : {};
    return {
      ok: Boolean(checkResult && checkResult.ok && parsed.ok === true && parsed.combinedStatus === 'pass')
    };
  }
  ...
  function normalizeBrowserBridgeCandidateTests(checkResult) {
    const parsed = checkResult && checkResult.parsed ? checkResult.parsed : {};
    return {
      ok: Boolean(checkResult && checkResult.ok && parsed.ok === true && Number(parsed.failed || 0) === 0)
    };
  }

  function runShadowRuntimeBoundaryReport(options) {
    const opts = Object.assign({
      projectRoot: typeof process !== 'undefined' ? process.cwd() : ''
    }, options || {});

    const diagnosticsReportRaw = runNodeCheck(opts.projectRoot, 'dev/run-event-v2-noop-hook-diagnostics-report.js');
    ...
    const shadowOnlyRuntimeBoundaryHarnessRaw = runNodeCheck(opts.projectRoot, 'dev/run-event-v2-shadow-only-runtime-boundary-harness.js');
    ...
    const normalized = {
      diagnosticsReport: normalizeDiagnosticsReport(diagnosticsReportRaw),
      ...
      shadowOnlyRuntimeBoundaryHarness: normalizeShadowOnlyRuntimeBoundaryHarness(shadowOnlyRuntimeBoundaryHarnessRaw),
    ...
    const result = Formatter.toConsolidatedResult(normalized);

    return Object.assign({}, result, {
      rawResults: {
        diagnosticsReport: diagnosticsReportRaw,
        hookAwareStaticCheck: hookAwareStaticCheckRaw,
        isolatedHookUnitHarness: isolatedHookUnitHarnessRaw,
        shadowOnlyRuntimeBoundaryHarness: shadowOnlyRuntimeBoundaryHarnessRaw,
  ...
  const api = Object.freeze({
    runShadowRuntimeBoundaryReport
  });

  globalScope.ShadowBridgeRuntimeBoundaryConsolidatedReport = api;
